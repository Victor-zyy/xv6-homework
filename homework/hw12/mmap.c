...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
  // Your code here.
	...
	static uintptr_t pgmap = 0;
	if(pgmap != 0){
		munmap((void *)pgmap, page_size);
	}
  uintptr_t fault_addr = (uintptr_t)si->si_addr;
	uintptr_t fpage_start = align_down(fault_addr, page_size);
	mmap((void *)fpage_start, page_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
setup_sqrt_region(void)
{
  ...
  if (sqrts == MAP_FAILED) {
	...
	printf("AS_LIMIT  = %d 0x%08x\n", AS_LIMIT, AS_LIMIT); 
	printf("MAX_SQRTS = %d 0x%08x\n", MAX_SQRTS, MAX_SQRTS); 
	printf("len       = %ld 0x%lx\n", MAX_SQRTS * sizeof(double) + AS_LIMIT, MAX_SQRTS * sizeof(double) + AS_LIMIT); 
